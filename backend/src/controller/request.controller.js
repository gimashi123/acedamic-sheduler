...
export const getRequestsByStatus = async (req, res) => {
  try {
    ...
    if (!Object.values(REQUEST_STATUS).includes(status)) {
      return errorResponse(res, 'Invalid status', HTTP_STATUS.BAD_REQUEST);
    }
...
export const updateRequestStatus = async (req, res) => {
  try {
    ...
    if (!request) {
      return errorResponse(res, 'Request not found', HTTP_STATUS.NOT_FOUND);
    }
    ...
    if (status === REQUEST_STATUS.APPROVED) {
      ...
      if (existingUser) {
        console.log(
          `User with email ${request.email} already exists. Skipping user creation.`,
        );
      } else {
...
Academic Scheduler Team`;
          ...
          if (emailResult.success) {
            console.log(
              `Approval email sent to ${request.email} using provider: ${emailResult.provider}`,
            );
          } else {
            console.warn(
              `Could not send approval email to ${request.email}: ${emailResult.message}`,
            );
          }
        } catch (emailError) {
...
Academic Scheduler Team`;
        ...
        if (emailResult.success) {
          console.log(
            `Rejection email sent to ${request.email} using provider: ${emailResult.provider}`,
          );
        } else {
          console.warn(
            `Could not send rejection email to ${request.email}: ${emailResult.message}`,
          );
        }
      } catch (emailError) {
...
const sendEmail = async (to, subject, text) => {
  ...
  if (process.env.EMAIL_ENABLED !== 'true' && !emailSettings.isEnabled) {
    console.warn(
      'Email notifications are disabled in settings. Skipping email sending.',
    );
    return false;
  }
